...
import jams.data.*;
import jams.model.*;
import org.unijena.j2k.mathematicalCalculations.*;

/**
 *
 * @author Peter Krause
 */
@JAMSComponentDescription(title = "J2KProcessSnow",
author = "Peter Krause",
...
public class J2KProcessSnow extends JAMSComponent {
    ...
    private boolean calcSnowAccumulation(double temp, double area, double critDens) {
        ...
        if (MathematicalCalculations.round(this.run_snowDepth, 9) == 0) {
            return true;
        }
        ...
        if (MathematicalCalculations.round(this.run_totDens, 9) > critDens) {
    ...
    private boolean calcMetamorphosis(double temp, double TRS, double temp_fac, double rain_fac, double ground_fac, double area, double SAC, double critDens) {
        ...
        /*test_totDens = this.run_totDens / critDens;
        if (test_totDens > 0.999999 && test_totDens < 1) {
        System.out.println("testtotdenspot" + test_totDens);//this.run_totDens = critDens;
        }*/
        //if(this.run_totDens >= critDens){
        if (MathematicalCalculations.round(this.run_totDens, 9) >= critDens) {
        ...
        /* test_totDens = this.run_totDens / critDens;
        if (test_totDens > 0.999999 && test_totDens < 1) {
        System.out.println("testtotdens" + test_totDens);
        this.run_totDens = critDens;
        System.out.println("-this.run_totDens" + this.run_totDens);
