...
public class MultiEntityReaderTS extends JAMSComponent {
    ...
    private void createTopology() throws Attribute.Entity.NoSuchAttributeException {
        ...
        StringTokenizer tokenizer_to_hru, tokenizer_weights;
<<<<<<< .mine
        ...
        Iterator<JAMSEntity> hruIterator, reachIterator;
=======
...
<<<<<<< .mine
        ArrayList<JAMSEntity> receiverHRUs, receiverReaches;
        ArrayList<Double> receiverHRUsWeights, receiverReachesWeights, receiverArea;
=======
        ...
        try {
            ...
            while ((s != null) && !s.startsWith("#")) {
                ...
                while (sTok.hasMoreTokens() && tschuessnull == false) {
                    double doubleID = Double.parseDouble(sTok.nextToken());
                    double doubleWeight = Double.parseDouble(tTok.nextToken());

                    sumWeight = 0.0001 * Math.round((sumWeight - doubleWeight) * 10000);

                    if ((doubleID == 0 && doubleWeight != 0) || (doubleID != 0 && doubleWeight == 0)) {
                        System.out.println("Fehler bei HRU " + doubleID + ". Anzahl der Empfaenger-HRUs und der Wichtungen stimmen nicht ueberein.");
                        JOptionPane.showMessageDialog(null, "Fehler bei HRU " + doubleID + ". Anzahl der Empfaenger-HRUs und der Wichtungen stimmen nicht ueberein.");
                        System.exit(-1);
                    }
                ...
                if (sumWeight >= 0.001) {
                    System.out.println("Fehler bei HRU " + eID + ". Summe der einzelnen Gewichte ungleich 1");
                    JOptionPane.showMessageDialog(null, "Fehler bei HRU " + eID + ". Summe der einzelnen Gewichte ungleich 1");
                    System.exit(-1);
                }
                    ...
                    if (e_ziel_to_hru.length > 0) {
                        ...
                        for (int i = 0; i < e_ziel_to_hru.length; i++) {
                            ...
                            if (e_ziel_to_hru[i] != null) {
                                ...
                                e_ziel_to_hru_bfl[i] = e_ziel.getDouble("BFLDouble") * e_ziel_to_hru_weights[i];
                                ...
                                if (e == e_ziel_to_hru[i]) {
                                    ...
                                    if (eBflMin != null && iZirkelMin != -1 && teilerMin != -1) {
                                        ...
                                        if (eBflMin_to_reach.length > 0) {
                                            ...
                                            System.out.println("HRU " + eBflMin.getDouble("ID") + ": Unterbrechung der Fliessbeziehung zu HRU " + eBflMin_to_hruId + ". Umleitung in das Fliessgewaessersegment -" + eBflMin_to_reachId + "; Beitragende Flaeche: " + Math.round(eBflMin.getDouble("BFLDouble") * eBflMin_to_hru_weights[iZirkelMin] / 1000.) / 1000. + " qkm");
                                        } else {
                                            ...
                                            System.out.println("HRU " + eBflMin.getDouble("ID") + ": Unterbrechung der Fliessbeziehung zu HRU " + eBflMin_to_hruId + ". Verteilung des Wassers auf alle anderen Fliessbeziehungen der HRU;" + " Beitragende Flaeche: " + Math.round(eBflMin.getDouble("BFLDouble") * eBflMin_to_hru_weights[iZirkelMin] / 1000.) / 1000. + " qkm");
                                        }
                                        ...
                                        JOptionPane.showMessageDialog(null, "Nicht aufloesbarer Zirkel! Fliessbeziehung von HRU " + e_ziel.getDouble("ID") + " zu HRU " + e_ziel_to_hru[i].getDouble("ID") + " kann nicht unterbrochen werden.");
                                    }
                                    continue marke;
                                }
