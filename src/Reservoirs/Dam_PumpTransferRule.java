...
public class Dam_PumpTransferRule extends JAMSComponent {
    ...
    @JAMSVarDescription(
            access = JAMSVarDescription.AccessType.READ,
            description = "Water volume pumped out of the reservoir"
    ...
    public Attribute.DoubleArray PumpDam;

    @JAMSVarDescription(
            access = JAMSVarDescription.AccessType.READ,
            description = "Threshold till which abstraction commences",
            defaultValue = "0",
            unit = "L",
            lowerBound = 0,
            upperBound = Double.POSITIVE_INFINITY
    )
    public Attribute.Double damPumpAdapt;
    ...
    public Attribute.Double pumpProgram;
    ...
    @Override
    public void initAll() {
        if (PumpDam.getValue().length != 1 && PumpDam.getValue().length != 12) {
    ...
    public void run() {
        ...
        double _PumpOutDam;
        if (PumpDam.getValue().length > 1) {
            int nowmonth = time.get(Calendar.MONTH);
            _PumpOutDam = PumpDam.getValue()[nowmonth];
        } else {
            _PumpOutDam = PumpDam.getValue()[0];
        }

        if (damStorage.getValue() > 0) {
            if (damStorage.getValue() <= ((damPumpAdapt.getValue()
