...
public class J2KProcessReachRouting_Tracking_V1 extends JAMSComponent {
    ...
    public Attribute.Double TracerFromReachRD1;
    ...
    public Attribute.Double TracerFromReachRD2;
    ...
    public Attribute.Double TracerFromReachRG1;
    ...
    public Attribute.Double TracerFromReachRG2;
    ...
    public Attribute.Double TracerFromReachSimRunoff;
    ...
    public void run() {
        ...
        /* AJOUT 
        Sum of incoming flows for local total runoff calculation*/
        double inTot = inRD1.getValue() + inRD2.getValue() + inRG1.getValue() + inRG2.getValue() + inAddIn.getValue();
...
//        getModel().getRuntime().println("Processing reach: " + ID);
        ...
        double TrackedFromReachRD1 = this.TracerFromReachRD1.getValue();
        double TrackedFromReachRD2 = this.TracerFromReachRD2.getValue();
        double TrackedFromReachRG1 = this.TracerFromReachRG1.getValue();
        double TrackedFromReachRG2 = this.TracerFromReachRG2.getValue();
        double TrackedFromReachSimRunoff = this.TracerFromReachSimRunoff.getValue();
        
        double destReachTrackedFromReachRD1 = 0;
        double destReachTrackedFromReachRD2 = 0;
        double destReachTrackedFromReachRG1 = 0;
        double destReachTrackedFromReachRG2 = 0;
        double destReachTrackedFromReachSimRunoff = 0;
        
//        getModel().getRuntime().println("Ici");

        // Save volume and transferring it if tracked reach
        if (ReachTracked == ID){
            //getModel().getRuntime().println(ID + " est bien le Reach tracé !");
            
            TrackedFromReachRD1 = RD1out;
            TrackedFromReachRD2 = RD2out;
            TrackedFromReachRG1 = RG1out;
            TrackedFromReachRG2 = RG2out;
            TrackedFromReachSimRunoff = cumOutflow;
            
//            getModel().getRuntime().println("C'est bien le Reach tracé. Volume sauvegardé et transmis au reach " + DestID);
//            getModel().getRuntime().println("Le volume sauvegardé et transferé est : " + TrackedFromReachRD2);


            destReachTrackedFromReachRD1 =   TrackedFromReachRD1;         
            destReachTrackedFromReachRD2 =   TrackedFromReachRD2;         
            destReachTrackedFromReachRG1 =   TrackedFromReachRG1;         
            destReachTrackedFromReachRG2 =   TrackedFromReachRG2;         
            destReachTrackedFromReachSimRunoff =   TrackedFromReachSimRunoff;          
            //getModel().getRuntime().println("TracerFromReachSimRunoff = " + TrackedFromReachSimRunoff);

            DestReach.setDouble("TracerFromReachRD1", destReachTrackedFromReachRD1);
            DestReach.setDouble("TracerFromReachRD2", destReachTrackedFromReachRD2);
            DestReach.setDouble("TracerFromReachRG1", destReachTrackedFromReachRG1);
            DestReach.setDouble("TracerFromReachRG2", destReachTrackedFromReachRG2);
            DestReach.setDouble("TracerFromReachSimRunoff", destReachTrackedFromReachSimRunoff);
            
            TracerFromReachRD1.setValue(TrackedFromReachRD1);
            TracerFromReachRD2.setValue(TrackedFromReachRD2);
            TracerFromReachRG1.setValue(TrackedFromReachRG1);
            TracerFromReachRG2.setValue(TrackedFromReachRG2);
            TracerFromReachSimRunoff.setValue(TrackedFromReachSimRunoff);
            
            
            
        } else if (this.TracerFromReachSimRunoff.getValue() != 0  && DestReach != null){            /* Calculation and transfer of tracked volume*/
            
            //getModel().getRuntime().println("Transfert de l'écoulement à " + DestID);
                      
//            getModel().getRuntime().println("Pour " + ID + ",TrackedRD2 = "+ TrackedFromReachRD2+ ", inRD1 = " + inRD1 + " et outRD1 = " + RD1out);

            TrackedFromReachRD1 = (TrackedFromReachRD1*RD1out)/inRD1.getValue();
            TrackedFromReachRD2 = (TrackedFromReachRD2*RD2out)/inRD2.getValue();
            TrackedFromReachRG1 = (TrackedFromReachRG1*RG1out)/inRG1.getValue();
            TrackedFromReachRG2 = (TrackedFromReachRG2*RG2out)/inRG2.getValue();
            TrackedFromReachSimRunoff = (TrackedFromReachSimRunoff*cumOutflow)/inTot;
//            getModel().getRuntime().println("Calcul...");
            
//            getModel().getRuntime().println("La partie de 2600 qui va dans  " + DestID + " est "+ destReachTrackedFromReachRD2);
            
            if(inRD1.getValue() == 0){
                TrackedFromReachRD1 = 0;
            }
            if(inRD2.getValue() == 0){
                TrackedFromReachRD2 = 0;
            }
            if(inRG1.getValue() == 0){
                TrackedFromReachRG1 = 0;
            }
            if(inRG2.getValue() == 0){
                TrackedFromReachRG2 = 0;
            }
            if(inTot == 0){
                TrackedFromReachSimRunoff = 0;
            }
            ...
            //getModel().getRuntime().println("Le volume de " + ReachTracked + " qui va dans " +  DestID + " est : " + TrackedFromReachSimRunoff);
        } else{
            TrackedFromReachRD1 = (TrackedFromReachRD1*RD1out)/inRD1.getValue();
            TrackedFromReachRD2 = (TrackedFromReachRD2*RD2out)/inRD2.getValue();
            TrackedFromReachRG1 = (TrackedFromReachRG1*RG1out)/inRG1.getValue();
...
//getModel().getRuntime().println("TracerFromReachSimRunoff = " + TracerFromReachSimRunoff);
