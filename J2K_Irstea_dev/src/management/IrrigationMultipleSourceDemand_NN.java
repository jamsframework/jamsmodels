...
public class IrrigationMultipleSourceDemand_NN extends JAMSComponent {
    ...
    @JAMSVarDescription(
            access = JAMSVarDescription.AccessType.READ,
            description = "Reach/Subbasin ID (reach extraction) for irrigation source (get from HRULoop). - parameter"
    )
    public Attribute.Double irrig_source_ID_reach;
    
    @JAMSVarDescription(
            access = JAMSVarDescription.AccessType.READ,
            description = "HRU ID (ground water extraction) for irrigation source (get from HRULoop). - parameter"
    )
    public Attribute.Double irrig_source_ID_GW;
    ...
    public void run() {
        ...
        if ((actET.getValue() / potET.getValue()) < etDeficit.getValue()) {
            ...
            if (run_irrigationSourceTypeConfiguration == 1 || run_irrigationSourceTypeConfiguration == 4 || run_irrigationSourceTypeConfiguration == 6){
                Attribute.Entity hru = run_hruMap.get((long) irrig_source_ID_GW.getValue());
                //add the current HRU to the list of HRUs to be irrigated by that HRU/Subbasin
                if (!hru.existsAttribute(irrigationGWEntitiesListName.getValue())) {
                    hru.setObject(irrigationGWEntitiesListName.getValue(), new ArrayList<Attribute.Entity>());
                }
                List<Attribute.Entity> l1 = (List) hru.getObject(irrigationGWEntitiesListName.getValue());
                l1.add(run_hruID);
            }
