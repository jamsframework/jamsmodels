...
//    public Attribute.Double farmdamMaxExtraction;
    ...
    @JAMSVarDescription(
            access = JAMSVarDescription.AccessType.READWRITE,
            description = "Water volume from RD1 stored in Device",
            unit = "L"
    )
    public Attribute.Double farmdam_actRD1;
    
    @JAMSVarDescription(
            access = JAMSVarDescription.AccessType.READWRITE,
            description = "Water volume from RD2 stored in Device",
            unit = "L"
    )
    public Attribute.Double farmdam_actRD2;
    
    @JAMSVarDescription(
            access = JAMSVarDescription.AccessType.READWRITE,
            description = "Water volume from RG1 stored in Device",
            unit = "L"
    )
    public Attribute.Double farmdam_actRG1;
    
    @JAMSVarDescription(
            access = JAMSVarDescription.AccessType.READWRITE,
            description = "Water volume from RG2 stored in Device",
            unit = "L"
    )
    public Attribute.Double farmdam_actRG2;
...
//            getModel().getRuntime().sendInfoMsg("HRU "+ run_currentHRU.getId() +" has no Attribute " + irrigationFarmdamEntitiesListName.getValue());
            return;
        }
...
//        double run_farmdamMaxExtraction = farmdamMaxExtraction.getValue();
        ...
        double run_farmdam_actRD1 = farmdam_actRD1.getValue();
        double run_farmdam_actRD2 = farmdam_actRD2.getValue();
        double run_farmdam_actRG1 = farmdam_actRG1.getValue();
        double run_farmdam_actRG2 = farmdam_actRG2.getValue();
...
//            getModel().getRuntime().sendInfoMsg("HRU "+ run_currentHRU.getId() +": "+ run_totalDemand + 
//                    " L demanded (" + run_totalDemandRemaining +" from farmdam), " + run_totalAvailable + " L available");
//        }
        ...
        if ((run_totalAvailable != 0) && (run_totalDemandRemaining != 0)){ // if there is water available (in and/or act) AND demanded for irrigation
            ...
            // Storage update
            double run_newFarmdamStorage = run_farmdamStorage - run_totalExtraction;
            double run_remainingFarmdamFraction = 0;
            if (run_farmdamStorage > 0) {
                run_remainingFarmdamFraction = run_newFarmdamStorage / run_farmdamStorage;
            }
            run_farmdam_actRD1 *= run_remainingFarmdamFraction;
            run_farmdam_actRD2 *= run_remainingFarmdamFraction;
            run_farmdam_actRG1 *= run_remainingFarmdamFraction;
            run_farmdam_actRG2 *= run_remainingFarmdamFraction;
            
            farmdamStorage.setValue(run_newFarmdamStorage);
            farmdam_actRD1.setValue(run_farmdam_actRD1);
            farmdam_actRD2.setValue(run_farmdam_actRD2);
            farmdam_actRG1.setValue(run_farmdam_actRG1);
            farmdam_actRG2.setValue(run_farmdam_actRG2);
            
            
//            getModel().getRuntime().sendInfoMsg("Extracted: "+ run_totalExtraction + " L from farmdam");
