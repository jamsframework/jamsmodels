...
public class Dam_Inflow_Reach_ extends JAMSComponent {
...
//    public Attribute.Double releaseProgram;
    ...
    public void run() {
        ...
        // Total inflow to the dam
        double potInflow
                = riverInflowRD1.getValue()
                + riverInflowRD2.getValue()
                + riverInflowRG1.getValue()
                + riverInflowRG2.getValue();
        ...
        // Available storage in the dam
        double headroom = Math.max(0,
                damCapacity.getValue() - damStorage.getValue());
        ...
        // Maximum amount that can enter the dam
        double maxInflow = Math.min(headroom, potInflow);

        // Fraction of the reach flow entering the dam
        double frac = maxInflow / potInflow;

        // Apply user-defined maximum transfer fraction
        frac = Math.min(frac, maxTransferFrac.getValue());

        // Safety limits
        frac = Math.max(0.0, Math.min(1.0, frac));

        // Remove transferred water from the river
        riverInflowRD1.setValue(riverInflowRD1.getValue() * (1.0 - frac));
        riverInflowRD2.setValue(riverInflowRD2.getValue() * (1.0 - frac));
        riverInflowRG1.setValue(riverInflowRG1.getValue() * (1.0 - frac));
        riverInflowRG2.setValue(riverInflowRG2.getValue() * (1.0 - frac));

        // Store transferred water in the dam
