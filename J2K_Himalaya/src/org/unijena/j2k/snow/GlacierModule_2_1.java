...
@JAMSComponentDescription(
        ...
        "integrating melt correction factor to include slope and aspect in radiation based model//s.nepal."
        )
    
    public class GlacierModule_2_1 extends JAMSComponent {
    ...
    public void run() throws JAMSEntity.NoSuchAttributeException, IOException {
        ...
        if (meltTemp > this.tbase.getValue()) {
            if (this.meltFormula.getValue() == 1) {
                //simple formula
                //snow melt
                snowMelt = (1 / n) * this.ddfSnow.getValue() * (meltTemp - this.tbase.getValue());
                ...
                //complex formula
                //snow melt
                snowMelt = (1 / n) * (this.meltFactor.getValue() + this.alphaSnow.getValue() * this.radiation.getValue()) * (meltTemp - this.tbase.getValue());
                snowMelt = snowMelt * this.area.getValue();
                if (snowMelt >= snowStor) {
                    snowMelt = snowStor;
                    snowStor = 0;
                ...
                //ice melt only when no snow is available
                if(snowStor == 0){
                    iceMelt = (1 / n) * this.meltFactor.getValue() + this.alphaIce.getValue() * this.radiation.getValue() * (meltTemp - this.tbase.getValue());
                    iceMelt = iceMelt * this.area.getValue();
                }
                else{
                    iceMelt = 0;
...
//the input to the module is from rain and snowmelt, hence, total snowmelt also inlcudes rain component
        //try to segregate the q_snow into rain and snowmelt and see if the two sums up qsnow)
        //test_snowmelt
        //test_rain
        //ice routing
        double q_snow1 = snowRunofftm1.getValue() * Math.exp(-1/this.kSnow.getValue()) + snowMelt * (1-Math.exp(-1/this.kSnow.getValue()));
