...
@JAMSComponentDescription(
    ...
    @JAMSVarDescription(
            access = JAMSVarDescription.AccessType.READ,
            description = "sealed grade adaptation Factor",
            defaultValue = "1"
            )
            public Attribute.Double sealedGrade_AF;
   ...
   @JAMSVarDescription(
    ...
    @JAMSVarDescription(
            access = JAMSVarDescription.AccessType.READ,
            description = "maximum infiltration rate in summer for one time step ADAPTATION FACTOR",
            defaultValue = "1",
            unit="-"
            )
            public Attribute.Double soilMaxInfSummer_AF;
    
    @JAMSVarDescription(
            access = JAMSVarDescription.AccessType.READ,
            description = "maximum infiltration rate in winter for one time step ADAPTATION FACTOR",
            defaultValue = "1",
            unit="-"
            )
            public Attribute.Double soilMaxInfWinter_AF;
    
    @JAMSVarDescription(
            access = JAMSVarDescription.AccessType.READ,
            description = "maximum infiltration rate on snow for one time step ADAPTATION FACTOR",
            defaultValue = "1",
            unit="-"
            )
            public Attribute.Double soilMaxInfSnow_AF;
 ...
 //           public Attribute.Double soilDistMPSLPS2;
    ...
    public void run() {
        ...
        this.calcInfImperv(sealedGrade.getValue() * sealedGrade_AF.getValue());
    ...
    private double calcMaxInfiltration(int nowmonth){
        ...
        if(this.run_snowDepth > 0)
            maxInf = this.soilMaxInfSnow.getValue() * soilMaxInfSnow_AF.getValue() * this.run_area;
        else if((nowmonth >= 5) & (nowmonth <=10))
            maxInf = (1 - this.run_satSoil) * soilMaxInfSummer.getValue() * soilMaxInfSummer_AF.getValue() * this.run_area;
        else
            maxInf = (1 - this.run_satSoil) * soilMaxInfWinter.getValue() * soilMaxInfWinter_AF.getValue() * this.run_area;
