 ...
 */
@JAMSComponentDescription(title = "J2KProcessLayerdSoilWaterBog",
        author = "Peter Krause, modifications by Manfred Fink",
        description = "Calculates soil water balance for each HRU with vertical layers, swiched off layer diffusion")
public class J2KProcessLayeredSoilWaterBog extends JAMSComponent {
...
//        netSnow.setValue(this.run_inSnow);
    ...
    private boolean layer_diffusion() {
        /*bog version commented diffusion switched of*/
        ...
        if (MobileWater > 0) {
            ...
            if (hor == nhor - 1) {
                ...
                if (this.run_vertComp > maxPerc) {
                    ...
                    //Bog version not all water goes lateral but write back into FPS and higher layers

                    int i = hor;
                    while (i >= 0) {
                        double kappa = this.run_maxLPS[i] - this.run_actLPS[i];

                        if (kappa >= rest) {
                            this.run_actLPS[i] = this.run_actLPS[i] + rest;
                            rest = 0;
                        } else {
                            this.run_actLPS[i] = this.run_actLPS[i] + kappa;
                            rest = rest - kappa;
                        }
                        i--;
                    }
                    if (rest > 0){
                        this.run_actDPS = this.run_actDPS + rest;
                    }
                    //original
