...
description = "Description, erster Test mit Infiltration aus Reach in Grundwasserleiter.")
public class J2KProcessGroundwater_D_N_1_v03 extends JAMSComponent {
    ...
    double run_maxRG1, run_maxGW, run_actRG1, run_actGW, run_inRG1, run_inGW, run_outRG1, run_outGW, run_genRG1, run_genGW,
    ...
    public void run() throws JAMSEntity.NoSuchAttributeException {
        ...
        // Percolation wird gelesen (geht später direkt in GW)
        ...
        this.distRG1_GW();             // in Abhängigkeit der Topographie wird das Perkolationswasser zwischen RG1 und GW verteilt
        ...
        this.updategwTable();           // Berechnung des potentiellen GW-Standes, wenn outGW tatsächlich abgegeben werde sollte
    ...
    private boolean calcDarcyGWout() throws JAMSEntity.NoSuchAttributeException {
        ...
        //if (this.run_gwTableUpper >= this.run_gwTableLower) {
            if (toPoly.getValue() != null) {
                    ...
                    double gwFlowArea = this.run_gwFlowWidth * (this.run_gwTableUpper - this.run_baseHeigth);
                    run_cf = gwFlowArea * this.run_Kf_geo / this.run_gwFlowLength;

                    GW_out_m3 = run_cf * gwDifference;
                    //[m³/s]   = [m²]       * [m/s]           * [m]          / [m]
                ...
                if (gwDifference < 0) {
                gwDifference = 0;
                getModel().getRuntime().println("Negativer Gradient zum Reach! Entity:" + actualEntityID);
                double H = 5 * ((this.run_gwTableLower - this.run_baseHeigth) + (this.run_gwTableUpper - this.run_baseHeigth));  //Hier Summe der Wasserstände in HRU und Reach
                ...
                GW_in_m3 = 0.5 * alphaL * this.alphaC.getValue() * gwDifference * (-1) * toReach.getDouble("length");
                ...
                double U = toReach.getDouble("width") + 2 * (toReach.getDouble("waterTable_NN") - toReach.getDouble("MEAN_H"));                                               //Hier benetzter Umfang
                double alphaL = (2 * H * (run_Kf_geo)) / (this.gwFlowLength.getValue() + (2 * H / Math.PI) * Math.log(2 * H / (Math.PI * U / 2)));
                GW_out_m3 = 0.5 * alphaL * this.alphaC.getValue() * gwDifference * toReach.getDouble("length");
                }
                if (GW_out_m3 > potOutflow) {
                    GW_out_m3 = potOutflow;
        ...
        GW_out = GW_out_m3 * 86400 * 1000;     //[l/d]
        
        if (gwDifference < 0) {
             GW_in = GW_in_m3 * 86400 * 1000;     //[l/d]
             
             double ReachStage = toReach.getDouble("channelStorage");
             if (ReachStage >= GW_in) {
        	this.run_actGW = this.run_actGW + GW_in;
                } else {
                GW_in = ReachStage;
                this.run_actGW = this.run_actGW + GW_in;
                
                }
             toReach.setDouble("channelStorage", ReachStage - GW_in);
             GW_out = 0;
        }
