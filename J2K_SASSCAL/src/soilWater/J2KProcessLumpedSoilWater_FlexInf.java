/*
 * J2KProcessLumpedSoilWater_FlexInf.java
...
@VersionComments(entries = {
    ...
    @VersionComments.Entry(version = "1.0_1", date = "2018-09-06", comment = "Modified/fixed calculation of MPS inflow")
})
public class J2KProcessLumpedSoilWater_FlexInf extends JAMSComponent {
    ...
    @JAMSVarDescription(
            access = JAMSVarDescription.AccessType.READ,
            description = "Maximum infiltration rate for one time step. This "
                    + "attribute expects 12 values, one for each month.",
            lowerBound = 0.0,
            upperBound = 100.0,
            defaultValue = "50.0",
            unit = "mm"
    )
    public Attribute.Double[] soilMaxInfiltration;
    ...
    public void run() {
         ...
         */
        double maxInf = this.calcMaxInfiltration(time.get(Attribute.Calendar.MONTH));
        ...
        /**
         * Distribution of MobileWater to the lateral (interflow) and vertical
         * (percolation) flowpaths
         */
    ...
    private double calcMaxInfiltration(int nowmonth) {
        ...
        if (this.run_snowDepth > 0) {
            ...
            maxInf = (1 - this.run_satSoil) * soilMaxInfiltration[nowmonth].getValue() * this.run_area;
        }
    ...
    private double calcMPSInflow(double infiltration) {
        ...
        // therefore we will use a very small value instead
        double _satMPS = Math.max(run_satMPS, 1E-7);

        // calc potential input to actMPS
        double potInMPS = (inflow) * (1. - Math.exp(-1 * alpha / _satMPS));

        // limit the input to MPS to the available capacity
        double inMPS = Math.min(potInMPS, this.run_maxMPS - this.run_actMPS);

        // add the water to MPS and remove it from the inflow
